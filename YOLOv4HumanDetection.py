import sys
sys.path.insert(0,'Users\SteveAhn\Documents\HumanDetection\HumanDetectionp37')
import cvlib
import cv2
...
import smtplib
import ssl   # for email alerts
from email.message import EmailMessage
import imghdr
from devtokens import DEV_TWILIO_TO, DEV_TWILIO_TOKEN, DEV_TWILIO_FROM, DEV_TWILIO_SID

# img and vid extensions
...
# function takes a file name (full path), checks that file for human shaped objects and saves the frame into 'save_directory'
def humanChecker(video_file_name, save_directory, yolo = "yolov4", continuous = False, nth_frame = 10, confidence = 0.65, gpu = False):

    global VALID_FILE_FLAG

    # flags for human match or file error
    is_human_found = False
    analyze_error = False
    is_valid = False

    # increment every time a person is detected for file naming
    person_detection_counter = 0

    # check if image
    if os.path.splitext(video_file_name)[1] in IMG_EXTENSIONS:
        frame = cv2.imread(video_file_name)   # frame will contain just an image

        # image validation
        if frame is not None:
            frame_count = 8  # necessary so our for loop runs below
            nth_frame = 1
    ...
    if is_valid:
        # look at every nth_frame of our video file, run frame through detection_common_objects
        # Increase 'nth_frame' to examine fewer frames and increase speed but could reduce accuracy
        for frame_number in range(1, frame_count - 6, nth_frame):
            ...
            # feed or frame (or image) into detect_common_objects, not utilize gpu default
            try:
...
def twilioAlertSender(TWILIO_TOKEN, TWILIO_SID, TWILI_FOM, TWILIO_TO):
    # if script called with --twilio and person is detected, send SMS notification
    client = Client(TWILIO_SID, TWILIO_TOKEN)
    client.messages.create(body = f"Human Detected: {HUMAN_DETECTION_FLAG} \n Valid Files Examined: {VALID_FILE_FLAG} \n Errors Detected: {ERROR_FLAG}", from_ = TWILIO_FROM, to = TWILIO_TO )
...
def emailAlertSender(save_directory, SENDER_EMAIL, SENDER_PASS, RECEIVER_EMAIL):
    ...
    if HUMAN_DETECTION_FLAG is True:
        msg['Subject'] = "Possible Intruder Alert"

    elif HUMAN_DETECTION_FLAG is False and VALID_FILE_FLAG is True:
        msg['Subject'] = "No Humans Detected"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--twilio', action='store_true', default = False, help="Flag to use Twilio text notification")
    ...
    parser.add_argument('--continuous', action='store_true', help="This option will go through entire video file and save all frames with people, default is to stop at first instance of a person")
    parser.add_argument('--confidence', type=int, choices=range(1,100), default=65, help="Value 1-99 to determine confidence level for a hit, Too high may miss humans, too low may acquire false positives")
    parser.add_argument('--frames', type=int, default=10, help='Only examine every nth frame, default is 10')

    args = vars(parser.parse_args())

    # decide which model we'll use, default is yolov4, more accurate but takes longer
    if args['tiny_yolo']:
        yolo_string = 'yoolov4-tiny'
    else:
        yolo_string = 'yolov4'
    ...
    if args['twilio']:
        try:
            ...
            TWILIO_SID = DEV_TWILIO_SID
            TWILIO_FROM = DEV_TWILIO_FROM
            TWILIO_TO = DEV_TWILIO_TO
        except:
            print("Something went wrong with Twilio parameters, check your os.environ parameters.")
            sys.exit(1)

    # if the --email flag is used, this will look for environment variables regarding email
    if args['email']:
        try:
            ...
            print("Something went wrong with Email variables, check your os.environ parameters.")
            sys.exit(1)
    ...
    print(f"Continuous examination of video is set to {args['continuous']}")
    print('\n\n')
